...
### Added
- **Cover Editor (development)**: Edit Everything now provides a compact, resizable cover workspace that remembers its dimensions and temporarily incorporates the existing Full Video player. Persistent Play/Pause, frame-step (or one-second Shift-click), and Capture Frame controls remain together, current/proposed cover previews support upload and clipboard input, capture pauses automatically, and saving remains explicit.
