(function initializeFastTagHelp(root) {
    ...
    const GUIDE_SECTIONS = Object.freeze([
        ...
        { id: 'video', icon: '🎬', title: 'Video, Scrubbing and Pop-out', html: `
            ...
            <p>In Full Video mode, place the pointer over the video and use the mouse wheel or trackpad to scrub. Slow movements, normal scrolling, and fast flicks use configurable skip distances. Hold <kbd>Shift</kbd> to use the fine freeze step. You can also click or drag the progress bar at the bottom as a normal timeline; it preserves whether playback was previously playing or paused. Reset Defaults also resets the limited-display scrubbing onboarding cue.</p>
            ...
            <h3>Cover Editor</h3><p>Select the picture button in Edit Everything's video controls to open the compact Cover Editor. The existing player moves into the cover workspace and switches to Full Video where possible. An ordinary arrow click moves one second; Shift-click moves one frame. Hold either arrow briefly to repeat until it is released or the pointer moves away. Persistent Play/Pause, step, and Capture Frame controls remain on one row; Capture Frame pauses automatically. Resize from the bottom-right corner—FastTag remembers the size for the next opening. Next/previous scene navigation keeps the editor open at the same size and position and loads the incoming scene; an unsaved proposed cover must be discarded or navigation is cancelled. Explicitly closing the editor returns the player to its previous embedded or floating location and restores its previous mode.</p>
    ...
    function openGuide(options = {}) {
        ...
        const guideRoot = typeof overlay.attachShadow === 'function' ? overlay.attachShadow({ mode: 'open' }) : overlay;
        guideRoot.innerHTML = `
            <style>:host{color-scheme:${isDark ? 'dark' : 'light'}}*,*::before,*::after{box-sizing:border-box}</style>
            <div id="fasttag-help-dialog" role="dialog" aria-modal="true" aria-labelledby="fasttag-help-title" style="width:min(1080px,96vw);height:min(780px,92vh);display:flex;flex-direction:column;background:${colors.bg};color:${colors.text};border:1px solid ${colors.border};border-radius:12px;overflow:hidden;box-shadow:0 24px 70px rgba(0,0,0,.65);">
                <div class="fasttag-help-header" style="display:flex;align-items:center;gap:12px;padding:13px 16px;background:${colors.panel};border-bottom:1px solid ${colors.border};">
                    <div style="min-width:0;flex:1"><div id="fasttag-help-title" style="font-size:16px;font-weight:800;">⚡ FastTag User Guide</div><div style="font-size:10.5px;color:${colors.muted};">Version ${options.version || '4.4.2'} · Available offline</div></div>
        ...
        const nav = guideRoot.querySelector('#fasttag-help-nav');
        const content = guideRoot.querySelector('#fasttag-help-content');
        const search = guideRoot.querySelector('#fasttag-help-search');
