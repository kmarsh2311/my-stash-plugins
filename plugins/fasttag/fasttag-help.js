(function initializeFastTagHelp(root) {
    ...
    const GUIDE_SECTIONS = Object.freeze([
        ...
        { id: 'video', icon: '🎬', title: 'Video, Scrubbing and Pop-out', html: `
            ...
            <p>Alternatively, upload a JPEG/PNG/WebP image, drop an image file onto New Cover, or use a clipboard image. Review the proposed cover and select Set Cover. Closing with an unsaved proposed cover asks before discarding it, including when the parent popup is closed. On localhost or HTTPS, Paste can normally read the clipboard directly after browser permission. On an HTTP LAN address, select Paste and then press Ctrl+V or Cmd+V when prompted. This works with any LAN IP or hostname.</p>
        ` },
        ...
        overlay.innerHTML = `
            <div role="dialog" aria-modal="true" aria-labelledby="fasttag-help-title" style="width:min(1080px,96vw);height:min(780px,92vh);display:flex;flex-direction:column;background:${colors.bg};color:${colors.text};border:1px solid ${colors.border};border-radius:12px;overflow:hidden;box-shadow:0 24px 70px rgba(0,0,0,.65);">
                <header style="display:flex;align-items:center;gap:12px;padding:13px 16px;background:${colors.panel};border-bottom:1px solid ${colors.border};">
                    <div style="min-width:0;flex:1"><div id="fasttag-help-title" style="font-size:16px;font-weight:800;">⚡ FastTag User Guide</div><div style="font-size:10.5px;color:${colors.muted};">Version ${options.version || '4.2.13'} · Available offline</div></div>
